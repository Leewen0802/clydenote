...
class Talk(object):
    ...
    @loggedIn
    def sendTag(self, to, mid):
        aa = '{"S":"0","E":"4","M":'+json.dumps(mid)+"}"
        text = "已標記:"
        mention = "@SCT"
        text_ = mention
        return self.sendMessage(to,text_, contentMetadata={'MENTION':'{"MENTIONEES":['+aa+']}'}, contentType=0)

    """Operation"""
    def sendMessageWithFooter(self,to, text, name, url, iconlink):
        contentMetadata = {
            'AGENT_NAME': name,
            'AGENT_LINK': url,
            'AGENT_ICON': iconlink
    ...
    def sendMessageWithFooter(self,to, text):
        self.reissueUserTicket()
        dap = self.getProfile()
        ticket = "http://line.me/ti/p/"+self.getUserTicket().id
        pict = "http://dl.profile.line-cdn.net/"+self.pictureStatus
        name = dap.displayName
        dapi = {"AGENT_ICON": pict,
            "AGENT_NAME": name,
            "AGENT_LINK": ticket
            }
        self.sendMessage(to, text, contentMetadata=smartmoes)
    @loggedIn
    ...
    def sendMessagePlus(self, to, text, contentMetadata={},relatedMessageId=0,):
        msg = Message()
    ...
    @loggedIn
    def sendPeplyLocation(self, relatedMessageId, to, location, contentMetadata={}, contentType=15):
    ...
    def relatedMessage(self, to, text,relatedMessageId=None, contentMetadata={}):
        ...
        self._messageReq[to] += 1
        return self.talk.sendMessage(self._messageReq[to], msg)
    def GetSpeed(self, groupId, relatedMessageId):
        try:
            kickerMid = self.profile.mid
            time0 = timeit.timeit('"-".join(str(n) for n in range(100))', number=5000)
            str1 = str(time0)
            start = timeit.default_timer()
            self.say(groupId, "Test Speed", relatedMessageId)
            ...
            get_allgroup = self.getGroupIdsJoined()
            get_allgroup_time = time.time() - get_allgroup_time_start
            get_contact_time_start = time.time()
            get_contact = self.getContact(kickerMid)
            get_contact_time = time.time() - get_contact_time_start
            get_friend_time_start = time.time()
            get_friend = self.getAllContactIds()
            get_friend_time = time.time() - get_friend_time_start
            get_group_time_start = time.time()
            get_group = self.getGroup(groupId)
            get_group_time = time.time() - get_group_time_start
            self.say(groupId, '◈╾ 程序處理速度\n{}秒\n◈╾ 指令反應\n{}秒\n◈╾ 處理速度\n{}秒\n◈╾ 資料讀取速度\n    ◈╾ 讀取自己\n     %.10f秒\n    ◈╾ 讀取群組\n     %.10f秒\n    ◈╾ 讀取友資\n     %.10f秒\n    ◈╾ 讀取全部群組\n     %.10f秒\n    ◈╾ 讀取全部好友\n     %.10f秒'.format(str(loadtime), str(str1), str(xtime)) %(get_profile_time/3, get_group_time/3, get_contact_time/3, get_allgroup_time/3, get_friend_time/3), relatedMessageId)
        except Exception as e:
            raise e
    ...
    def Tag(self, to, midlist):
        ...
        b=[]
        for mid in midlist:
            b.append({"S":str(s), "E" :str(s+5), "M":mid})
            s += 7
            txt += u'@Moe \n'
    ...
    def sendMessage2(self, to, text, contentMetadata={}, contentType=0):
        ...
        self._messageReq[to] += 1
        return self.talk.sendMessage(self._messageReq[to], msg)
    def say(self, to, text, relatedMessageId, contentMetadata={}, contentType=0, messageRelationType=3, relatedMessageServiceCode=1):
        msg = Message()
        msg.to, msg._from = to, self.profile.mid
        msg.text = text
        msg.relatedMessageId = relatedMessageId
        msg.contentType, msg.contentMetadata, msg.messageRelationType, msg.relatedMessageServiceCode = contentType, contentMetadata, messageRelationType, relatedMessageServiceCode
        if to not in self._messageReq:
            self._messageReq[to] = -1
        self._messageReq[to] += 1
        return self.talk.sendMessage(self._messageReq[to], msg)
    ...
    def image_search2(self, query):
        print('searching')
        query = query.replace(' ', "+")
        url = "https://www.google.com/search?espv=2&biw=1366&bih=667&tbm=isch&oq=kuc&aqs=mobile-gws-lite.0.0l5&q=" + (query)
        res = requests.get(url)

        mozhdr = {'User-Agent': "Mozilla/5.0 (Windows NT 6.2; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1667.0 Safari/537.36'"}
        req = requests.get(url, headers = mozhdr)
        html = req.content
        ...
        images = random.choice(images)['ou']
        return images
    def forward(self, to):
    ...
    @loggedIn
    def _kickMember(self, groupId, midlist):
        return self.talk.kickoutFromGroup(0, groupId, midlist)
